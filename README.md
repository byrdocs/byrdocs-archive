...
只有作为正式出版物的教育资源才能归入 `books` 中。非出版物禁止归入 `books`。
...
关于 `ISBN码`，应**统一使用 ISBN-13 码**。有些外文书比较讲究，其 Print 版和 PDF 版有各自的 ISBN，这时**应统一使用 Print 版**；还有些外文书的 Bound 版和 Loose-leaf 版有各自的 ISBN，这时**应统一使用 Bound 版**；有些外文书有 Hardcover 版和 Paperback 版，这时优先使用 Hardcover 版。
